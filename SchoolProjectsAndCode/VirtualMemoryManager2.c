...
int findPage(int logAddy, char* PT, struct TLB *tlb,  char* PM, int* OF, int* pageFaults, int* TLBhits, int phys_mem){
	...
	if (TLBhit == false){
		...
		//	printf("Pagehit\t\t");
			...
			newFrame = readDisk(pageNum, PM, OF, phys_mem);
			PT[pageNum] = newFrame;
			(*pageFaults)++;
		}
		frame = PT[pageNum];
	...
	int index = ((unsigned char)frame*phys_mem)+offset;
	val = *(PM+index);
...
int readDisk (int pageNum, char *PM, int* OF, int phys_mem){
	...
	if (fseek(BS, pageNum * phys_mem, SEEK_SET)!=0)
		printf("error in fseek\n");

	if (fread(buffer, sizeof(char), phys_mem, BS)==0)
		printf("error in fread\n");
	

	int i = 0;
	for(i; i < phys_mem; i++){
		*((PM+(*OF)*phys_mem)+i) = buffer[i];
	}
...
int main (int argc, char* argv[]){
	...
	const int phys_mem = atoi(argv[2]);
	

	char PhyMem[phys_mem][phys_mem];
	...
	while (fscanf(fd, "%d", &val)==1){
		findPage(val, PageTable, &tlb, (char*)PhyMem, &openFrame, &pageFaults, &TLBhits, phys_mem);
		inputCount++;
	}
